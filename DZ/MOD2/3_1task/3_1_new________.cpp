...
public:
    ...
    int get_data(){return data;}        //  возвращает значение 
    void set_right(DecartTreeNode* val) {     //  меняем значение левого потомка 
        this->Right = val;
    };
    void set_left(DecartTreeNode* val){       // меняем занчение правого потомка
        this->Left = val;
    };
    void set_data(int val){ this->data = val;}
    DecartTreeNode<T> * get_left(){return Left;}      //  считываем занчение левого потомка

    DecartTreeNode<T> * get_right(){return Right;}    //  считываем значение правого потомка
    int get_priority() {return priority;}
    void set_priority() {this->priority = priority;};
...
public:
    ...
    void add(DecartTreeNode<T>*& PTR_root, DecartTreeNode<T>* PTR_insert){
        if (PTR_root == NULL){
            PTR_root = PTR_insert;
        }else if (PTR_root->get_priority() < PTR_insert->get_priority()) {
            Split (PTR_root, PTR_insert->get_data(), PTR_insert->Left, PTR_insert->Right); 
            PTR_root = PTR_insert;
        } else {
    ...
    void Split(DecartTreeNode<T>* PTR_root, int data, DecartTreeNode<T>*& PTR_left, DecartTreeNode<T>*& PTR_right){
        if (PTR_root == NULL){
            PTR_left = NULL;
            PTR_right = NULL;
        } else if (data < PTR_root->get_data()) {
            Split(PTR_root->get_left(), data, PTR_left, PTR_root->Left);  
            PTR_right = PTR_root;
        } else {
...
int main()
{
    ...
    std::cout << abs(treeHeight(MyTree.get_root()) -  treeHeight(MyDecartTree.get_root()));

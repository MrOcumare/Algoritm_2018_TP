...
class Queue {
    public:
        ...
        Queue( const Queue& ) = delete; // конструктор копирования 
        void operator=( const Queue& ) = delete; // Оператор присваивания копированием
        Queue& operator=(Queue&& other) = delete ; // Оператор присваивания перемещением
        Queue(Queue&& other) = delete; // Конструктор перемещения
